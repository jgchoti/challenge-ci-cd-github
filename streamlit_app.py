...
def create_analytics_dashboard(df):
    ...
    with col3:
        locations = df['place_of_residence'].nunique() if 'place_of_residence' in df.columns else 0
        st.markdown(f"""
        <div class="stat-container">
            <h3 style="color: #FF6B35; margin: 0;">📍</h3>
...
def display_pet_gallery(df):
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        search_term = st.text_input("🔍 Search pets", placeholder="Search by name, breed, or description...")
    with col2:
        size_options = ["All"] + list(df['size'].dropna().unique()) if 'size' in df.columns else ["All"]
        size_filter = st.selectbox("Filter by size", size_options)
    with col3:
        gender_options = ["All"] + list(df['gender'].dropna().unique()) if 'gender' in df.columns else ["All"]
    ...
    if gender_filter != "All" and 'gender' in filtered_df.columns:
        filtered_df = filtered_df[filtered_df['gender'].str.contains(gender_filter, case=False, na=False)]
    ...
    for i in range(0, len(filtered_df), cols_per_row):
        ...
        for j, (idx, pet) in enumerate(filtered_df.iloc[i:i+cols_per_row].iterrows()):
            if j < len(cols):
                with cols[j]:
...
def main():
    ...
    if df is None:
        st.warning("⚠️ No pet data available. Click 'Refresh Pet Data' to fetch fresh data.")
