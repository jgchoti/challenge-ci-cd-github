...
class Scraper:
    ...
    def setup_api_headers(self):
        ...
        common_config_str = urllib.parse.quote(json.dumps(common_config, separators=(',', ':')))
    ...
    def create_api_payload(self, offset=0, limit=20):
        ...
        return {
            ...
            "query": {
                "filter": {
                    "statusCheckBoxIfAdopted": {"$ne": True}
                },
        ...
        try:
            ...
            pet = {
                ...
                'other_fields': {k: v for k, v in data.items() if k not in [
                    'title', 'shortDescription', 'longDescription', 'link-animals-title',
                    'breed', 'size', 'gender', 'born', 'ageYear', 'placeOfResidence',
                    'image', 'picture_galery', 'added', 'lastModified', 'statusCheckBoxIfAdopted'
                ]}
            }
